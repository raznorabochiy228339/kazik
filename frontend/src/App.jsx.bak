      ...
      <Routes>
        ...
        <Route path="/case/:id" element={
          ...
          />
              ...
              <Route path="/crash" element={<CrashGame user={user} setUser={setUser} notify={notify} />} />
</Routes>

      
      <nav className="fixed bottom-0 left-0 right-0 bg-white/5 backdrop-blur p-3 border-t border-white/10 flex justify-around">
        <button onClick={()=>navigate('/')} className="flex flex-col items-center text-sm">
          <div>🧩</div><div>Кейсы</div>
        </button>
        <button onClick={()=>navigate('/inventory')} className="flex flex-col items-center text-sm">
          <div>🎒</div><div>Инвентарь</div>
        </button>
        <button onClick={()=>navigate('/crash')} className="flex flex-col items-center text-sm font-semibold">
          <div>✈️</div><div>Краш</div>
        </button>
      </nav>
